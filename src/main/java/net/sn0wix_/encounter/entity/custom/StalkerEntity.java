package net.sn0wix_.encounter.entity.custom;
...
public class StalkerEntity extends HostileEntity implements GeoEntity {
    ...
    private Vec3d handleScareOffSet() {
        ...
        Encounter.LOGGER.info(Math.sqrt(deltaX * deltaX + deltaZ * deltaZ) + " pos between points");
    ...
    private void scareOne(ServerPlayerEntity player) {
        ...
        scarePosVec = new Vec3d(finalDest.x, this.getEyeY() - player.getEyeHeight(EntityPose.STANDING) - 0.2, finalDest.z);
        Encounter.LOGGER.info(scarePosVec + " Original scare pos");
    ...
    public boolean tryAttack(Entity target) {
        if (target instanceof PlayerEntity && !getWorld().isClient && scareTicksLeft == 0) {
            ...
            if (squaredDistance <= d) {
                ...
                scareAll();
                this.getWorld().getPlayers().forEach(player -> ServerPlayNetworking.send((ServerPlayerEntity) player, ModPackets.LOCK_PLAYER, createBuf(scarePosVec)));
    ...
    private PacketByteBuf createBuf(Vec3d vec3d) {
